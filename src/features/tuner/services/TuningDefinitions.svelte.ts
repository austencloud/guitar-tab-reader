...
import { a4Calibration, adjustFrequencyForCalibration } from './CalibrationSettings.svelte';
...
// Create a reactive state class for tunings
class TuningState {
	#tunings = $state<Tunings>({ ...standardTunings });
	#selectedTuning = $state<string>(loadSelectedTuning());

	get tunings(): Tunings {
		return this.#tunings;
	}

	get selectedTuning(): string {
		return this.#selectedTuning;
	}

	set selectedTuning(value: string) {
		this.#selectedTuning = value;
		// Save immediately when changed
		saveSelectedTuning(value);
	}

	addCustomTuning(name: string, strings: StringDefinition[]): void {
		this.#tunings = { ...this.#tunings, [name]: strings };
	}
}

// Export a singleton instance
const tuningState = new TuningState();

// Export reactive getters - components should use these
export function getTunings(): Tunings {
	return tuningState.tunings;
}

export function getSelectedTuning(): string {
	return tuningState.selectedTuning;
}

export function setSelectedTuning(value: string): void {
	tuningState.selectedTuning = value;
}

export function addCustomTuning(name: string, strings: StringDefinition[]): void {
	tuningState.addCustomTuning(name, strings);
}

// For backward compatibility - export non-reactive values
// Components using these will need to be updated to use the getter functions
export const tunings = standardTunings;
export const selectedTuning = loadSelectedTuning();
...
export function getClosestString(
	...
	// Adjust strings for calibration
	const calibratedStrings = activeStrings.map((string) => ({
		...string,
		frequency: adjustFrequencyForCalibration(string.frequency, a4Calibration.value)
	}));

	let closest = calibratedStrings[0];
	let smallestDistance = Infinity;

	for (const string of calibratedStrings) {
	...
	// Find original string to return (without calibration adjustment)
	const closestIndex = calibratedStrings.indexOf(closest);
	return smallestDistance <= 0.26 ? activeStrings[closestIndex] : null;
}

export function calculateCents(actual: number, target: number): number {
	// Adjust target for calibration
	const calibratedTarget = adjustFrequencyForCalibration(target, a4Calibration.value);
	return Math.round(1200 * Math.log2(actual / calibratedTarget));
}
